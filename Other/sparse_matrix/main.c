...
int triangular(Spa_Mat *mat, int l, int c){
    int cont = 0;
    for(int i = 0; i < l; i++){
        if((buscar_pos(i, i, mat) != 0))
           cont += 1;
           }
    if ((cont == l) && (buscar_pos(0, 1, mat) != 0)){
        for(int i = 0; i < l; i++){
                for(int j = 0; j < i+1; j++){
                    if(buscar_pos(i, j,mat) != 0)
                        return 0;
                }
        }
        for(int i = 0; i < l; i++){
    ...
    else if ((cont == l) && (buscar_pos(1, 0, mat) != 0)){
        for(int i = 0; i < l; i++){
                for(int j = 1+ i; j < l; j++){
                    if(buscar_pos(i, j,mat) != 0)
                        return 0;
...
int main() {
    ...
    for (int i = 0; i < l; i++){
            for(int j = 0; j < c; j++)
                trocar(matriz[i][j], i, j, esparsa);
    }
    num = triangular(esparsa, l, c);
    if(num == 1)
        printf("triangular");
    else
