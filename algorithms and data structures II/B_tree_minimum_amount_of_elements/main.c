...
void min_elementos(NodeB *tree, int *contador, int *raiz){
    int i;
    if ((*raiz == 1)){
        *raiz = 0;
        if((tree != NULL) && (tree->nro_chaves == 1))
            *contador += 1;
    }
    else if ((tree != NULL) && (tree->nro_chaves == 2)){
            *contador += 1;
    }
    if(tree != NULL)
        for (i = 0; i <= tree->nro_chaves; i++)
            min_elementos(tree->filhos[i], contador, raiz);
}
